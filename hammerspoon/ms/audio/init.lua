local print = require('ms.logger').logger_fn('ms.audio.init')
...
--[[export]] local function set_volume(new_volume)
...
--[[export]] local function increase_volume(opt_delta)
  if opt_delta then
...
--[[export]] local function decrease_volume(opt_delta)
  if opt_delta then
    update_volume(-opt_delta)
  else
    local config = devices.get_output_config()
...
--[[export]] local function set_mute(muted)
